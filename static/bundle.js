...
window.onload = function () {
  ...
  function update() {
    var accel = player.body.acceleration.getMagnitude();
    ...
    if (player.action) player.animations.play(player.action);else if (player.body.speed > MAX_SPEED / 10) player.animations.play('run');else player.animations.play('stand');
    ...
    if (speedDiff > 300 || player.action) game.physics.arcade.collide(player, ball);else {
...
function registerKeyboard(game, player, ball) {
  ...
  Object.keys(cursors).forEach(function (key) {
    cursors[key].onHoldCallback = function () {
  ...
  space.onUp.add(function () {
    if (new Date().getTime() - player.lastTouch < 1000 || player.ballDist < 40) {
      ...
      player.kickCancel = setInterval(function () {
        return player.signal.dispatch({ kick: -1 });
      }, 1000);
    } else {
